...
exports.getSingleVideo = async (req, res) => {
    try {
        ...
        const video = await Video.findOne({ uploadID: videoId }).sort({ createdAt: -1 }).populate(["actor", "category", "tags", "views"]);

        if (!req.cookies.cookieVideoId?.includes(videoId)) {
            const views = await Views.findOneAndUpdate({ _id: video.views }, {
                $inc: { views: 1 },
            })
        }
...
exports.addVideo = async (req, res) => {
    try {
        ...
        const videoInfo = await axios.get(`https://api.streamsb.com/api/file/info?key=66418reck8nac228fzn2j&file_code=${response.data.result[0].code}`)
        ...
        await new Promise((resolve, reject) => {
            axiosResponse.data.pipe(fs.createWriteStream("file.jpg"))
                .on('error', reject)
        ...
        const response2 = await axios.put(`https://storage.bunnycdn.com/skbj/videos/${response.data.result[0].code}.jpg`, img, { 'content-type': 'application/x-www-form-urlencoded', headers: { "AccessKey": `aff796b2-9990-480b-b69778a60b10-b14c-4a29` } });
        ...
        const actor = await Actor.findOneAndUpdate({ _id: req.body.actor }, {
            $inc: { totalVideos: 1 },
        })
